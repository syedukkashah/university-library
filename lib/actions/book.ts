...
export async function getBorrowedBooks(userId: string) {
  try {
    ...
    return { success: true, data: JSON.parse(JSON.stringify(borrowedBooks)) };
  } catch (error) {
    console.log(error);
    return { success: false, error: "Error getting borrowed books" };
  }
}
...
export async function searchBooks({
  ...
  try {
    ...
    return {
      ...
      metadata: { totalPage, hasNextPage },
    };
  } catch (error) {
    console.log(error);
    return { success: false, error: "Error searching books" };
  }
}
